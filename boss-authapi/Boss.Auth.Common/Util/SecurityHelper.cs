...
namespace Boss.Auth.Common.Util
{
    ...
    {
        private static readonly string DESKey = "!#t&ntsc"; // 8位或者16位
        private static readonly string DESIv = "9po#jixd"; // 8位或者16位

        private static readonly string AESKey = "12345pkijuyhtgre"; // 16位或者32位
        private static readonly string AESIv = "p#kiy%tgfdcsxzwq"; // 16位或者32位
        ...
        public static byte[] AESEncrypt(string input, string key = "", string iv = "")
        {
            ...
            {
                 LoggerHelper.Error(ex,ex.Message);
            }
            return null;
        }
        ...
        {
            var byteArr = AESEncrypt(input, key, iv);
            if (byteArr != null)
            {
        ...
        {
            string md5Password = MD5ToHex(password);
            string encryptPassword =MD5ToHex(md5Password.ToLower() + salt).ToLower();
